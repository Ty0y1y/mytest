...
# -------------------------- 全局页面配置 + 紧凑版自定义CSS --------------------------
st.set_page_config(
    page_title="多功能综合网站（顶部下划线导航版）",
    page_icon="🌟",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# 自定义CSS：紧凑版下划线导航（减小所有留空）
st.markdown("""
    <style>
    /* 隐藏radio默认的圆圈按钮 */
    ...
    div[role="radiogroup"] {
        display: flex;
        gap: 0.8rem;  /* 大幅减小选项间距（原2rem） */
        padding: 0.2rem 0;  /* 减小上下内边距（原0.5rem） */
        border-bottom: 1px solid #e0e0e0;
        flex-wrap: wrap;
        margin-bottom: 0.2rem !important;  /* 容器底部间距 */
    }
    
    /* 导航选项基础样式 - 紧凑 */
    div[role="radiogroup"] label {
        padding: 0.2rem 0;  /* 减小选项上下内边距 */
        cursor: pointer;
        font-size: 15px;  /* 微调字体大小更紧凑 */
        font-weight: 500;
        color: #666;
        border-bottom: 2px solid transparent;
        transition: all 0.3s ease;
        margin: 0 !important;  /* 移除选项默认margin */
    }
    ...
    /* 优化整体间距 - 大幅减小留空 */
    .stRadio {
        margin-bottom: 0.2rem !important;  /* 原1rem，大幅减小 */
    }
    /* 减小标题和导航之间的分隔线间距 */
    hr {
        margin: 0.3rem 0 !important;  /* 原默认更大 */
    }
    /* 减小标题的底部间距 */
    h1 {
...
def show_home():
    ...
    with col4:
        st.info("🎶 简易音乐播放器：播放音乐，切换歌曲，展示专辑封面和歌手信息")
    with col5:
...
def show_music_player():
    ...
    with col2:
        st.write(current_music["song_name"])
        st.write(f"**歌手**：{current_music['singer']}")
...
def show_video_center():
    ...
    # 选项卡分播放区和信息区（核心组件：选项卡）
    tab1, tab2 = st.tabs(["视频播放", "剧集信息"])
    with tab1:
        st.write(video_data[st.session_state['ind']]["title"])
